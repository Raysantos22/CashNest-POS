...
class ProductRepository(
    ...
    private val visibilityDao: ProductVisibilityDao // Add this parameter
// Add application context to access SessionManager
    ...
    fun getAlignedVisibleProducts(): Flow<Map<Category, List<Product>>> {
        return combine(allCategories, getVisibleProducts()) { categories, visibleProducts ->
            val alignedMap = mutableMapOf<Category, List<Product>>()

            // Always add all visible products to "All" category
            alignedMap[Category(-1, "All")] = visibleProducts

            // Group visible products by their categories
            categories
                .filter { it.name != "All" }
                .forEach { category ->
            ...
            // Add uncategorized visible products to "Uncategorized" category
            val uncategorizedProducts = visibleProducts.filter { product ->
                categories.none { category ->
                    product.itemGroup.equals(category.name, ignoreCase = true)
                }
            }
    ...
    fun getAlignedProducts(): Flow<Map<Category, List<Product>>> {
        return getAlignedVisibleProducts()
    }
